...
public class ActionHealHP extends AbilityActionHandler {
    ...
    public boolean execute(Ability ability, AbilityModifierAction action, ByteString abilityData, GameEntity target) {
        ...
        if (owner instanceof EntityClientGadget ownerGadget) {
            ...
            logger.info("Owner {} has top owner {}: {}", ability.getOwner(), ownerGadget.getOwnerEntityId(), owner);
        }
        ...
        if (!action.ignoreAbilityProperty)
            abilityRatio += target.getFightProperty(FightProperty.FIGHT_PROP_HEAL_ADD) + target.getFightProperty(FightProperty.FIGHT_PROP_HEALED_ADD);
        ...
        logger.info("Healing {} without ratios\n", amountToRegenerate);
